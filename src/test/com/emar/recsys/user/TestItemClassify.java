...
//	ReduceMultipleOutputsDriver<Text, Text, Text, Text> reduceDriver;
	...
	private void initData() {
		Text value = new Text("add69c2f2fd282210bbbeaf076d2b8d9\u0001\u0001112.236.22.25\u00016bdd89e9-14c3-31ea-8e8f-658929b69911\u000120130522210018\u000182510\u0001115736\u00011\u0001141985\u0001Mozilla/5.0 (Windows NT 5.1) AppleWebKit/537.1 (KHTM, like Gecko) Chrome/21.0.1180.89 Safari/537.1"),
				...
				val3 = new Text("add69c2f2fd282210bbbeaf076d2b8d9\u0001\u0001112.236.22.25\u00016bdd89e9-14c3-31ea-8e8f-658929b69911\u000120130522210018\u000182510\u0001115736\u00011\u0001141985\u0001Mozilla/5.0 (Windows NT 5.1) AppleWebKit/537.1 (KHTM, like Gecko) Chrome/21.0.1180.89 Safari/537.1");
		Text moutk = new Text("6bdd89e9-14c3-31ea-8e8f-658929b69911"), 
				moutv = new Text("82510\u000120130522210018"),
				moutk2 = new Text("6a15edd3-12fa-32fb-8fd0-f1ab61d77756"),
				moutv2 = new Text("20130525211925\u000179821"),
				moutk3 = new Text("6bdd89e9-14c3-31ea-8e8f-658929b69911"), 
				moutv3 = new Text("82510\u000120130522210018");
		...
		indata.add(new Pair<LongWritable, Text>(new LongWritable(3), val3));
		
		mopairs.add(new Pair<Text, Text>(moutk, moutv));
		mopairs.add(new Pair<Text, Text>(moutk2, moutv2));
		mopairs.add(new Pair<Text, Text>(moutk3, moutv3));
	...
	public void setUp() {
		...
		conf.set("LEN", "10");
		conf.set("index_list", "5\t6");
		conf.set("SEPA", "\u0001");
		conf.set("index_time", "4");
		conf.set("index_uid", "0");
		conf.set("topk", "3");
		conf.set("io.serializations","org.apache.hadoop.io.serializer.JavaSerialization," 
		            + "org.apache.hadoop.io.serializer.WritableSerialization");
		
		ItemClassify.Map mapper = new ItemClassify.Map();
		ItemClassify.Reduce reducer = new ItemClassify.Reduce();
...
//				mapDriver.getCounters().findCounter(CDRCounter.NoSMSCDR).getValue());
	...
	public void testReduce() throws IOException {
        ...
        Pair<Text, List<Text>> pair = shuffledData.get(0), 
        		pair2 = shuffledData.get(1);  // pair3 = shuffledData.get(2);
        reduceDriver.withInput(pair.getFirst(), pair.getSecond())
        		.withInput(pair2.getFirst(), pair2.getSecond())
//        		.withInput(pair3.getFirst(), pair3.getSecond())
        		.withOutput(new Text("6bdd89e9-14c3-31ea-8e8f-658929b69911"), 
        				new Text("{82510=2}"));
