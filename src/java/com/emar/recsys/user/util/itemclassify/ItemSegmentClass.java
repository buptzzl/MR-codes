...
public class ItemSegmentClass {
	...
	public ItemSegmentClass(Context context) throws IOException, URISyntaxException {
//		addClassCacheFile(context.getConfiguration());// 必须在new Job前调用
		this.init(context);
	}
	/** 加载 hadoop在137主节点上的 Cache文件  */
	public static void addClassCacheFile(Configuration conf) throws URISyntaxException {
	...
	public static void Load2HashMap(Vector<String> vec_line,
		...
		for (int i = 0; i < vec_line.size(); i++) {
			...
			if (!hashDiction.containsKey(result[0]))
				hashDiction.put(result[0], result[1]); // save the word and
														// the
			// pos
		}
	}
	...
	// @ADD static.
	public static void LoadCateGender(Vector<String> vec_con,
	...
	// @MOD static 加载配置文件
	public static void loadconfig(String filename, Vector<String> vec_config)
			throws IOException {
		BufferedReader br = new BufferedReader(new FileReader(filename));
		String aKeyWord;
		while ((aKeyWord = br.readLine()) != null) {
	...
	/**  按行读取一个文件的内容 到 vector.  */
	public static void loadcontent(String filename, Vector<String> vec_config)
			throws IOException {
		BufferedReader br = new BufferedReader(new FileReader(filename));
		String line;
		line = br.readLine();
	...
	// @MOD static 加载类目信息
	public static void LoadCateInfo() throws UnsupportedEncodingException,
			IOException {
		Vector<String> vec_con = new Vector<String>();
	...
	/**
	 * 将 vector中的每个原子按Tab分割，去第1部分为 词。
	 * @param pos 指定的词性
	 * @param hashDiction 词-类目 映射
	 * @param hsdiction 词-词性 映射
	 */
	public static void LoadWord23(Vector<String> vec_line, String pos,
			HashMap<String, CateInfo> hashDiction,
			HashMap<String, String> hsdiction) throws IOException {
		...
		for (int i = 0; i < vec_line.size(); i++) {
			...
			if (!hashDiction.containsKey(str) && str.length() > 1) {
				hashDiction.put(str, ccateinfo); // save the word and the
													// pos
	...
	// @MOD static.
	public static String QuanJiao2XiaoXie(String str) {
		String xiaoxie = "";
	...
	public static boolean isLeftShumingHao(String inputstr) {
		...
		for (int i = 0; i < inputstr0.length();) {
			...
			if (parameter.isNumeric(subword)) { // 从i开始是数字
				...
				if (firstendpos != (i + 1) && firstendpos != -1) {
					...
					if (firstendpos + 1 <= inputstr0.length()) {
						String firstword = inputstr0.substring(firstendpos,
								firstendpos + 1); // 取出这个非数字字符
						if (firstword.equals(".")) // 如果该数字字符是小数点号
						{
							// 从该小数点号下一个位置开始找到第一个非数字字符的位置
							int endpos = parameter.subNumpos2(inputstr0,
									firstendpos + 1);
							if (endpos != firstendpos + 1) {
								String newword2 = inputstr0
										.substring(i, endpos);
								...
								String newword2 = inputstr0.substring(i,
										firstendpos);
								vec_str.add(newword2);
								wordtag.put(newword2, "cm");
								i = firstendpos;
						...
						else // 不是小数点符号，则表明是一个整数
						{
							...
							String lastword = inputstr0.substring(i,
									firstendpos);

							if (firstendpos + 1 < inputstr0.length()) {
								String nextwordh0 = inputstr0.substring(
										firstendpos, firstendpos + 1);
								if (nextwordh0.endsWith("年")) {
									word = inputstr0.substring(i,
											firstendpos + 1);
									vec_str.add(word);
									wordtag.put(word, "t");
									i = firstendpos + 1;
			...
			if (parameter.isOrderWord(subword)) // 如果这个词是“第”
			{
				if (i + 1 < inputstr0.length() && i + 2 < inputstr0.length()) {
					...
					{
						int pos01 = subHanZiSuZi(inputstr0, i + 1); // int
																	// pos0
						// =
						// subHanZiPos(inputstr0,i);
						if (pos01 + 1 < inputstr0.length()) {
							String nextword0 = inputstr0.substring(pos01,
									pos01 + 1);
			...
			if (subword.equals("百") || subword.equals("千")
					|| subword.equals("万")) // 如果这个词是“百”或者是“千”或者是“万”
			{
				if (i + 1 < inputstr0.length() && i + 2 < inputstr0.length()) {
						...
						if (i + 3 < inputstr0.length()) {
							...
							if (nexword0.equals("之")) {
								...
								if (i + 4 < inputstr0.length()) {
									String nexword00 = inputstr0.substring(
											i + 3, i + 4);
									if (isHanZiSuZi(nexword00) == 1) // 如果是汉字数字
									{
										int pos000 = subHanZiSuZi(inputstr0,
												i + 4); // int
														// pos0
														// =
										// subHanZiPos(inputstr0,i);
...
//				System.out.println(" the is " + vec_con.elementAt(i));
	...
	public String ClassifySentence(String sentence) {
		...
		lastsentence = parameter
				.CutSentenceBySymbol(sentence, vec_restsentence);// lastsentence表示的是正文部分
		...
		String result = "";
		/*
		 * for(int i=0;i<vec_wordpos.size();i++) { result
		 * =vec_wordpos.elementAt(
		...
		for (int i = vec_wordpos.size() - 1; i >= 0; i--) {
			...
			if (hsCateProbInfo.containsKey(word)) {
				...
				for (int j = 0; j < ccateprobinfo.veccateprob.size(); j++) {
					String cateword = ccateprobinfo.veccateprob.elementAt(j)
							.GetCate();
					double prob = ccateprobinfo.veccateprob.elementAt(j)
							.GetProb();
	...
	public String GetCateByBrand(String brandword) {
		...
		// if(hsBrand2Cate.containsKey("可口可乐"))
		// System.out.println("包含可口可乐");
		String cate = "";
	...
	public String GetHighestGrade(Vector<String> vec_cateid) {
		...
		int sum = num_of_shuichan + num_of_meishi + num_of_shushi
				+ num_of_waimai + num_of_haiwei + num_of_ganhuo
				+ num_of_xiangchang;// 混合求和，
