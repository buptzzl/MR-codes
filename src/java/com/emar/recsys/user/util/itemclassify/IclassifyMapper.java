...
public class IclassifyMapper extends Mapper<LongWritable, Text, Text, Text> {

	// 定义Map & Reduce 中通用的对象
	private static final String MR_sepa = "\u0001";
	private static final String MR_kcfmt = "c_%s", MR_kufmt = "u_%s",
			MR_Isepa = "\t", MR_Iunuse = "*";
	...
	private static enum Counters {
		Useless, UnClass, UnMout, UnCamp, UnDate, UnClassify, EmpPName, EmpPPrice
	};
	...
	public void map(LongWritable key, Text val, Context context) {
		...
		if (atom.length < LenLog) {
			...
			String pname = atom[IdxPName].trim();
			// TODO 引入Util 类中的商品分类
			String[] pclass = this.ClassifyGoods(pname).split("\t");
			if (pclass.length != 5 || pclass[IdxClass].equals(MR_Iunuse)) {
				context.getCounter(Counters.UnClass).increment(1);
			} else {
				String pcnt = atom[IdxPCnt].trim();
				try{
					Integer.parseInt(pcnt);
				} catch (NumberFormatException e) {
					pcnt = VDefPCnt;
				}
				String pprice = atom[IdxPPrice].trim();
				if(pprice.length() == 0) {
					pprice = VDefPPrice;
				}
				String time = atom[IdxDate].trim();
				if(time.length() != 12){
					time = VDefPTime;
				}
				String host = VDefHost;
				if(atom.length > IdxHost && atom[IdxHost].trim().length() != 0) {
					host = atom[IdxHost];
				}

				try { // 同时以 plat-userid, campid 输出
					okey.set(String.format(MR_kufmt, platuser));
					oval.set(time + MR_sepa + pclass[IdxClass] + MR_sepa + pclass[IdxClassInfo] 
							+ MR_sepa + pname + MR_sepa + pprice + MR_sepa + pcnt + MR_sepa + host);
					context.write(okey, oval);

					okey.set(String.format(MR_kcfmt, campid));
					context.write(okey, oval);
				} catch (Exception e) {
