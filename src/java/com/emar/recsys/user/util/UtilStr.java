...
public class UtilStr {
	private static Logger log = Logger.getLogger(UtilStr.class);
		...
		try {
			try {
				...
				try {
					...
					if (i == res.length())
						return res;
					tmp = URLEncoder.encode(res.charAt(i) + "", GBK)
							.toLowerCase();
				} catch (UnsupportedEncodingException e1) {
				}
			}
		} catch (Exception e) {
			log.error("url decode failed. [MSG]: " + e.getMessage());
			return myurl;
		}
	...
	/**
	 * 嵌套解析多层数组为多层list， 解析 [[20130113152937, 50011980, 乳液, 卸洗凝胶, 102.4, ]] 注意：
	 * 保证每次的 arr 中的内容已经清空。
	 */
	public static int str2list(String str, String left, String right,
		...
		if (ibeg == -1) {
			...
			for (iend = getRBound(tstr, left, right, sepa, ibeg + 1), ++pdeep; ibeg != -1
					&& iend != -1; iend = getRBound(tstr, left, right, sepa,
					ibeg + 1)) {
	...
	private static int getRBound(String in, String left, String right,
			String sepa, int beg) {
		int N = 1, Nleft = 0;
		...
		while (N != 0) {
			while (iend != -1
					&& (iend + right.length() != in.length() && in.indexOf(
							sepa, iend + 1) != (iend + right.length()))) {
				beg = iend + 1;
				iend = in.indexOf(right, beg);
			}
			if (iend == -1) // 无匹配符号
		...
		return iend;
		/*
		 * // 右边界位置： 1 终点， 2 分隔符前出现 且 中间的左右边界符已经匹配 int ibeg = 0, iend = 0, nbeg;
		 * ibeg = getLBound(in, left, sepa, beg); // 下一个左边界 iend =
		 * in.lastIndexOf(right); if (ibeg == -1 || iend == -1) { if (iend ==
		 * (in.length() - right.length()) || in.indexOf(sepa, iend) == (iend +
		 * right.length())) return iend; return -1; } ibeg = beg; int cnt_in =
		 * 1, cnt_mid = -1; while (cnt_in != 0) { // nbeg = in.indexOf(left,
		 * ibeg + 1); nbeg = getLBound(in, left, sepa, ibeg + 1); // 在两个
		 * 前缀分割符之间，执行最长匹配 iend = nbeg == -1 ? in.lastIndexOf(right, in.length())
		 * : in.lastIndexOf(right, nbeg); // iend = in.indexOf(right, nbeg == -1
		 * ? ibeg + 1); if (iend == -1 || iend == (in.length() -
		 * right.length())) return iend; // error or last-char. cnt_mid =
		 * Math.min(UtilStr.SubStrCnt(in.substring(ibeg + 1, iend), left),
		 * UtilStr.SubStrCnt(in.substring(ibeg + 1, iend), right)); if (cnt_mid
		 * == 0 && (iend == (in.length() - right.length()) || in.indexOf(sepa,
		 * iend) == (iend + right.length()))) { --cnt_in; } else { cnt_in =
		 * cnt_mid; // 中间还有 mid 可左边界待匹配 ibeg = iend; } }
		 * 
		 * return iend;
		 */
	}
