...
//			obj = new JSONObject();
		...
		public Object parseLine(String line) {
			// 解析 IntPair 对象。
			final String sepa = "\t", sepa_com = ", ", sleft = "[", sright = "]";
			final int Ifemale = 0, Imale = 1, Iuid = 2, Ifea = 3, Iclass = 4;
...
//			List<String> ftmp = new ArrayList<String>();
			...
			classes.clear();
			int flag = UtilStr.str2list(atom[Iclass], sleft, sright, sepa_com, classes);
		...
		public String[] getClassLevel(int level) {
			if(level < 0 || this.classes.size() < (level + 1))
				return null;
			return this.classes.get(level).toArray(new String[this.classes.get(level).size()]);
		}
		...
		public String toString() {
			if (features == null || classes == null)
				return "ParseOrder\t"+getAttribute()+"\nfeature="+features+"\nclass="+classes
						+"\nuid="+uid+"\tdebug="+debug;
			return String.format("%s%s\t%s\nclass=%s\nuser=%s\ndebug=%s", getAttribute(), getClassify(), 
					Arrays.asList(getFeatures()), classes.toString(), uid, debug+"");
		}
	...
	public static void main(String[] args) {
		...
		String in = "D:/Data/MR-codes/data/test/good_sex.txt", 
				out = "D:/Data/MR-codes/data/test/good_sex.f.txt";
		
		try {
			pord.init(ClassType.BinarySex.toString());
			String line;
			System.out.println("[Info] attr=" + pord.getAttribute());
			DataInputStream fr = new DataInputStream(new BufferedInputStream(new FileInputStream(in)));
			while((line = fr.readLine()) != null) {
				pord.parseLine(line);
				System.out.println("[Info] class=" + pord.getClassify());
				System.out.println("[Info] feature=" + pord.getFeatures().length);
			}
			
			
			/// 反射测试代码
			Object ipar = (ParseLine) Class.forName("com.emar.recsys.user.model.ParseLine").newInstance();
